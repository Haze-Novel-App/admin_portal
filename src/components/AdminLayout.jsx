...
export default function AdminLayout() {
  ...
  const handleLogout = async () => {
    try {
      ...
      // The App.jsx listener will detect the session change, 
      // but navigating explicitly is a good safety measure.
      navigate('/');
    } catch (error) {
      ...
      <aside className={styles.sidebar}>
        ...
        <div style={{ marginTop: 'auto' }}>
          <NavItem
            icon={<LogOut size={20} />}
            label="Logout"
            onClick={handleLogout} // Link the logout function here
          />
        </div>
      </aside>

      {/* Main Area (Header and Content remains the same) */}
      <div className={styles.main}>
        <header className={styles.topbar}>
          <h1 className={styles.pageTitle}>Overview</h1>
          <div className={styles.profileSection}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ width: 40, height: 40, background: '#7C3AED', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
                <User size={20} />
              </div>
            </div>
          </div>
...
// NavItem component remains as you provided
function NavItem({ icon, label, active, onClick }) {
  return (
    <div
      onClick={onClick}
